...
| `difficulty` | ⬜       | The difficulty pool to attempt to browse the page from, `high` (coming soon), `medium`, or `low`; `low` is the default difficulty                                                                                                                   |
| `speed`      | ⬜       | The target speed to browse at, `light`, `ridiculous`, or `ludicrous` (planned), where `ridiculous` is an average of 30 percent faster than `light`; `light` is the default speed                                                                    |
...
| `readiness`  | ⬜       | The standard ready event to await before snapshotting the browsed content, `load` or `domcontentloaded`; `load` is the default event                                                                                                                |
| `delay`      | ⬜       | The number of seconds after the ready event to delay before snapshotting the browsed content, from `.1` to `10` inclusive; no delay is injected by default                                                                                          |
| `expiration` | ⬜       | The age in days to consider cached content expired at, where `0` disables caching; `1` is the default number of expiration days                                                                                                                     |
