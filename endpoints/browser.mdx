...
| `speed`      | ⬜       | The target speed to browse at, `light`, `ridiculous`, or `ludicrous` (planned), where `ridiculous` is N₁× faster on average than `light` and `ludicrous` is N₂× faster; `light` is the default speed                                                |
