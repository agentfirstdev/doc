...
Searches are performed live by default or can be queued for [later retrieval](scheduling). Live
searches take an average of a few seconds but may require multiple retries, in which case up to
**120 seconds** is allocated per API request.
