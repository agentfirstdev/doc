...
iconType: light
description: Extract live Google search results.
---
...
**Agent First’s** search service has a RESTful interface that accepts `GET` requests at
...
Searches are performed in real time by default or can be queued for [later retrieval](queuing).
Real-time searches take an average of a few seconds but may require multiple retries, in which case
up to **120 seconds** is allocated per API request.

### Parameters

In addition to the geotargeting and queuing parameters mentioned above, required and optional search
parameters can be appended with a
